...
class PatientRecordContract extends Contract {
    ...
    */
    ...
    async createPatientRecord(ctx,username,name,dob,gender,blood_type){
        ...
        //TASK 0: Add patient record by calling the method in the PRecordList
        await ctx.patientRecordList.addPRecord(precord); //see context constructor above
    }
    ...
    async getPatientByKey(ctx, username, name){
        ...
        return JSON.stringify(await ctx.patientRecordList.getPRecord(precordKey));
    }
    ...
    async updateCheckupDate(ctx,username,name,last_checkup_date){
        ...
        let record = await ctx.patientRecordList.getPRecord(precordKey);
        record.setLastCheckupDate(last_checkup_date);
        await ctx.patientRecordList.updatePRecord(record);
    ...
    //      Pass the Query string built to queryWithQueryString
        let queryString = {
            "selector": {
                ...
                "use_index": ["bloodTypeIndexDoc", "bloodTypeIndexName"]
        }
        return await this.queryWithQueryString(ctx, JSON.stringify(queryString));
    }
    ...
    //      Pass the Query string built to queryWithQueryString
        let queryString = {
            "selector": {
                ...
                "$or": [{"blood_type": blood_type1}, {"blood_type": blood_type2 }]
            },
